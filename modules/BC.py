...
def baseline0(y, degree=2, power=1, method="Powell",
    ...
    nchunk = int(y.size/chunksize)
    if nchunk <2:
        bl = bcL1(y, degree=degree, power=power, method=method)
    else:
        lsize = int(y.size/nchunk)
        ...
        if nbcores:             # Parallelization
            ...
            # print("i*lsize-cov: {0}, (i+1)*lsize+cov :{1} ".format(i*lsize-cov,(i+1)*lsize+cov))
            # print("type(degree) : {0}, type(power) : {1} ".format(type(degree), type(power)))
            # print("type(nchunk) : {0}".format(type(nchunk)))
